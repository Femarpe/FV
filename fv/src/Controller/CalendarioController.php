...
class CalendarioController extends AbstractController
{
    ...
    public function eventos(EntityManagerInterface $em): JsonResponse
    {
        ...
        foreach ($eventos as $evento) {
            $data[] = [
                'id' => $evento->get_id(),
                'title' => $evento->get_titulo(),
                'start' => $evento->get_fecha()->format('Y-m-d'),
                'color' => match ($evento->get_tipo()) {
                    'disponible' => '#40c057',
                    'no_disponible' => '#fa5252',
                    default => '#339af0'
                },
                'tipo' => $evento->get_tipo(),
                'isOwner' => $evento->get_usuario() === $this->getUser()
            ];
        }
    ...
    public function crear_evento(Request $request, EntityManagerInterface $em): JsonResponse
    {
        try {
            ...
            $evento->set_usuario($usuario);

            if ($tipo === 'evento') {
                $evento->set_titulo($data['titulo'] ?? 'Evento sin título');
                $evento->set_descripcion($data['descripcion'] ?? null);
            } else {
                $evento->set_titulo($tipo === 'disponible' ? 'Disponible' : 'No disponible');
            }
    ...
    #[Route('/api/evento/{id}', name: 'api_evento_editar', methods: ['PUT'])]
        public function editar_evento(int $id, Request $request, EntityManagerInterface $em): JsonResponse
        {
            ...
            if ($evento->get_usuario() !== $this->getUser()) return new JsonResponse(['error' => 'No autorizado'], 403);

            $data = json_decode($request->getContent(), true);
            $nuevoTitulo = $data['titulo'] ?? null;
            if (!$nuevoTitulo) return new JsonResponse(['error' => 'Título requerido'], 400);

            if ($evento->get_tipo() === 'evento') {
                $evento->set_titulo($nuevoTitulo);
                $em->flush();
            }
